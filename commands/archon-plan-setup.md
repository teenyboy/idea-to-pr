...
2. **If plan already in workflow artifacts**: Use `.claude/skills/idea-to-pr/artifacts/plan.md`

```bash
# Check if plan was created by archon-create-plan in this workflow
if [ -f ".claude/skills/idea-to-pr/artifacts/plan.md" ]; then
  PLAN_PATH=".claude/skills/idea-to-pr/artifacts/plan.md"
...
If `$ARGUMENTS` is an issue URL or number (e.g., `#123`), fetch the issue body instead (requires issue tracker CLI).
...
```bash
git remote get-url origin 2>/dev/null || echo "local (no remote)"
```
...
Write to `.claude/skills/idea-to-pr/artifacts/plan-context.md`:
...
Context written to: `.claude/skills/idea-to-pr/artifacts/plan-context.md`
